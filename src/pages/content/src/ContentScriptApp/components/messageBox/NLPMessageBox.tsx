...
import { displaCy } from "@src/shared/utils/displacy";
...
export default function MessageBox({
  ...
  // @ts-ignore
  const [elem, setElem] = useState<Node | undefined>(displacy.render(nlp));

  const svg = useRef<HTMLDivElement>(null);

  useEffect(() => {
    if (svg.current && elem) {
      svg.current.appendChild(elem);
    }
  }, [svg, elem]);
    ...
    >
      <MessageBoxContainer width={width} ref={containerRef} {...restProps}>
        <Stack>
          ...
          <HStack>
            <div style={{ marginLeft: "-32px" }} ref={svg}></div>
